...
describe('resolveAuthRedirect', () => {
  it('sends an invited user to Confirmation, even without a display_name', () => {
    expect(resolveAuthRedirect({ isInvited: true, isRecovery: false, next: '/admin/today' })).toBe(
      CONFIRMATION_ROUTE,
    );
  });
  ...
  it('sends a non-invited user with a recovery in window to update-password', () => {
    expect(resolveAuthRedirect({ isInvited: false, isRecovery: true, next: '/client/home' })).toBe(
      '/update-password',
    );
  });

  it('sends a confirmed user to the next destination', () => {
    expect(resolveAuthRedirect({ isInvited: false, isRecovery: false, next: '/client/home' })).toBe(
      '/client/home',
    );
  });
